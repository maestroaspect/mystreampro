...
#include <boost/filesystem.hpp> //MyStreamPro 2023-01-16 (Added)
...
void LLModelPreview::loadModelCallback(S32 loaded_lod)
{
    ...
    if (mFMP)
    {
        if (!mBaseModel.empty())
        {
            // MyStreamPro 2023-01-16 (Commented)
            // const std::string& model_name = mBaseModel[0]->getName();
            // ===
			
            // MyStreamPro 2023-01-16 (Added)
            boost::filesystem::path pathObj(mLODFile[loaded_lod]);
            const std::string& model_name = pathObj.stem().string();
            // ===
